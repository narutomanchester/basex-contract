...
contract BribeFactoryV3 is OwnableUpgradeable {
    ...
    constructor() {}
    function initialize(address _voter, address _permissionsRegistry, address[] calldata _defaultRewardToken) initializer  public {
        __Ownable_init();   //after deploy ownership to multisig
        voter = _voter;

        //bribe default tokens
        for (uint i = 0; i < _defaultRewardToken.length; i++) {
            defaultRewardToken.push(_defaultRewardToken[i]);
        }
