...
contract VoterV3 is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint internal constant DURATION = 2 minutes;                   // rewards are released over 7 days
    uint public VOTE_DELAY;                                     // delay between votes in seconds
    uint public constant MAX_VOTE_DELAY = 2 minutes;               // Max vote delay allowed
    ...
    /// @dev    the function is called by the minter each epoch. Only the minter can update rewards.
    /// @param  amount  amount to distribute
    function notifyRewardAmount(uint amount) external {
        require(msg.sender == minter);
        _safeTransferFrom(base, msg.sender, address(this), amount);     // transfer the distro in
        uint _totalWeight = totalWeightAt(_epochTimestamp() - 120);   // minter call notify after updates active_period, loads votes - 2 mins
        uint256 _ratio = 0;
    ...
    function _updateForAfterDistribution(address _gauge) private {
        address _pool = poolForGauge[_gauge];
        uint256 _time = _epochTimestamp() - 120;
    ...
    function forceResetTo(uint _tokenId) external VoterAdmin {
        lastVoted[_tokenId] = _epochTimestamp() - 60;
    }
